...
// DATA STORE (matches attempt.html style)
export const globalNodeState = new Map(); // nodeKey -> { activeFilters: Set }

function getOrCreateNodeState(key) {
    if (!globalNodeState.has(key)) {
        globalNodeState.set(key, { activeFilters: new Set() });
    }
    return globalNodeState.get(key);
}
...
/**
 * HELPER: Updates the visible window of items for a virtualized node
 */
...
/**
 * HELPER: Updates the virtual scrollbar visual state
 */
function updateVirtualScrollBar(node) {
    ...
    const thumbH = Math.max(15, ratio * availH);
...
/**
 * HELPER: Performs virtual scrolling
 */
function virtualScrollBy(node, delta) {
    ...
    if (diagram) diagram.model.startTransaction("scroll");
    updateWindow(node, constrainedIndex + BUFFER);
    if (diagram) diagram.model.updateTargetBindings(data);
    if (diagram) diagram.model.commitTransaction("scroll");
...
const _addDiagramEvents = (diagram) => {
    ...
    diagram.addDiagramListener("InitialLayoutCompleted", () => {
        ...
        // DETECT ACTUAL ROW HEIGHT dynamically
        const firstNode = diagram.nodes.first();
        if (firstNode) {
            const table = firstNode.findObject("TABLE");
            const scroller = firstNode.findObject("SCROLLER");
            ...
            if (scroller) {
                ...
                if (up && down) {
                    BUTTONS_H = up.actualBounds.height + down.actualBounds.height;
                }
            }
            ...
            if (node.data.isVirtualized) {
                ...
                if (thumb) {
                    ...
                    thumb.actionMove = (ev, thb) => {
                        ...
                        const range = bar.actualBounds.height - uph - downh;
                        const ratio = Math.max(0, Math.min(1, (local.y - uph) / range));
                ...
                if (bar) {
                    bar.click = (ev, br) => {
                        ...
                        const range = br.actualBounds.height - uph - downh;
                        const ratio = Math.max(0, Math.min(1, (local.y - uph) / range));
                ...
                const up = scroller.findObject("UP");
                if (up) up.click = (ev) => { ev.handled = true; virtualScrollBy(node, -1); };
                const down = scroller.findObject("DOWN");
                if (down) down.click = (ev) => { ev.handled = true; virtualScrollBy(node, 1); };

            } else {
                const table = scroller.findObject("TABLE");
                const full = node.data.fullItems || [];
                if (table) {
                    const availH = scroller.actualBounds.height;
                    const contentH = full.length * ROW_HEIGHT;
                    if (contentH <= availH) {
                        const bar = scroller.findObject("SCROLLBAR");
                        if (bar) bar.visible = false;
                    } else if (scroller._updateScrollBar) {
                        scroller._updateScrollBar(table);
                    }
                }
            }
        });
    ...
    diagram.addDiagramListener("PartResized", (e) => {
        ...
        if (obj.name === "SCROLLER") {
            ...
            diagram.model.startTransaction("resize scroll");

            // 1. Snapping logic (Accounting for scrollbar buttons)
            const currentH = obj.actualBounds.height;
            const contentH = currentH - BUTTONS_H;
            const snappedContentH = Math.round(contentH / ROW_HEIGHT) * ROW_HEIGHT;
            const snappedH = snappedContentH + BUTTONS_H + 4; // Add buffer

            if (Math.abs(currentH - snappedH) > 0.1) {
                obj.height = snappedH;
            }
            node.ensureBounds();

            if (node.data.isVirtualized) {
                // 2. VIRTUALIZED NODES: Recalculate window and align
                const trackH = snappedH - BUTTONS_H;
                const newMetrics = getViewportMetrics(node, trackH);
                const currentTopIndex = node.data.startIndex + table.topIndex;
                const newMaxIndex = Math.max(0, node.data.fullItems.length - newMetrics.capacity);
                const targetTopIndex = Math.min(currentTopIndex, newMaxIndex);

                updateWindow(node, targetTopIndex + BUFFER);
                diagram.updateAllTargetBindings("visibleItems");
                table.topIndex = Math.max(0, targetTopIndex - node.data.startIndex);
                updateVirtualScrollBar(node);
            } else {
                // 3. STANDARD NODES: Just align scrollbar
            ...
            diagram.model.commitTransaction("resize scroll");

            // Dual-pass sync to catch any rendering lag
            if (node.data.isVirtualized) {
                const latestTrackH = obj.actualBounds.height - BUTTONS_H;
                const latestMetrics = getViewportMetrics(node, latestTrackH);
                const newMaxIndex = Math.max(0, node.data.fullItems.length - latestMetrics.capacity);
                const targetTopIndex = Math.min(node.data.startIndex + table.topIndex, newMaxIndex);
...
function InitDiagram({ linkPresent }) {
    ...
    // OVERRIDE ResizingTool to snap during drag
    diagram.toolManager.resizingTool.computeResizing = function (box, min, max, cell, resizable, reshape, symmetric) {
        let result = go.ResizingTool.prototype.computeResizing.call(this, box, min, max, cell, resizable, reshape, symmetric);
        if (this.adornedObject && this.adornedObject.name === "SCROLLER") {
            const h = result.height;
            const contentH = h - BUTTONS_H;
            const snappedContentH = Math.round(contentH / ROW_HEIGHT) * ROW_HEIGHT;
            result.height = snappedContentH + BUTTONS_H + 4; // Add buffer to prevent cut-off
        }
        return result;
    };
    ...
    diagram.nodeTemplate =
        $(go.Node, "Auto",
            ...
            $(go.Shape, "Rectangle", { fill: "white", stroke: "#ccc", strokeWidth: 1 }),
                            ...
                            $(go.Panel, "TableRow",
                                ...
                                $(go.Panel, "Horizontal",
                                    ...
                                    new go.Binding("visible", "", (data, obj) => {
                                        ...
                                        // USE STORED CAPACITY for perfect sync during scroll/resize
                                        const capacity = nData.capacity || 10;
                                        const windowEnd = windowStart + capacity;

                                        // Only show the row contents if it's within the viewport's capacity range
                                    ...
                                    $(go.Shape, {
                                        ...
                                        click: (e, obj) => {
                                            ...
                                            if (node && attrData) {
                                                ...
                                                if (state) {
                                                    ...
                                                    // Refresh only the window because the "Always Rendered" set changed
                                                    const table = node.findObject("TABLE");
                                                    const currentTop = node.data.startIndex + (table ? table.topIndex : 0);
                                                    updateWindow(node, currentTop + BUFFER);

                                                    // Refresh bindings for the node data (to update visibleItems)
                                                    model.updateTargetBindings(node.data);
                                                    
                                                    // CRITICAL: Force update and synchronize topIndex to prevent jump
                                                    if (table) {
                                                        table.topIndex = Math.max(0, currentTop - node.data.startIndex);
                                                    }
                                                    
                                                    // Refresh the specific row's bindings to show the new filter state
                                        ...
                                        const node = obj.part;
                                        const state = node ? getOrCreateNodeState(node.key) : null;
