...
const globalNodeState = new Map(); // nodeKey -> { activeFilters: Set }
...
function updateWindow(node, centerIndex) {
    ...
    const { windowSize } = getViewportMetrics(node);
...
const _addDiagramEvents = (diagram) => {
    ...
    diagram.addDiagramListener("PartResized", (e) => {
        ...
        if (obj.name === "SCROLLER") {
            ...
            const snappedH = Math.round(currentH / ROW_HEIGHT) * ROW_HEIGHT;
            ...
            if (node.data.isVirtualized) {
                ...
                const newMaxIndex = Math.max(0, node.data.fullItems.length - metrics.capacity);
                ...
                table.panel.updateTargetBindings();
                node.ensureBounds();

                table.topIndex = Math.max(0, targetTopIndex - node.data.startIndex);
                updateVirtualScrollBar(node);
            } else {
                const capacity = Math.max(1, Math.ceil(obj.actualBounds.height / ROW_HEIGHT));
                const maxTopIndex = Math.max(0, table.rowCount - capacity);

                if (table.topIndex > maxTopIndex) {
                    table.topIndex = maxTopIndex;
                }
                                    ...
                                    $(go.Shape, {
                                        ...
                                        click: (e, obj) => {
                                            ...
                                            if (node && attrData) {
                                                ...
                                                if (state.activeFilters.has(attrData.name)) {
                                                    state.activeFilters.delete(attrData.name);
                                                } else {
                                                    state.activeFilters.add(attrData.name);
                                                }
                                                // Refresh only the bindings for this specific attribute row
                                                node.diagram.model.updateTargetBindings(attrData);
